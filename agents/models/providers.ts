...
const aiGatewayForProvider =
  ({ accountId, gatewayId, provider }: AIGatewayOptions) => `https://gateway.ai.cloudflare.com/v1/${accountId}/${gatewayId}/${provider}`

export type ProviderFactory = (opts: AIGatewayOptions) => LanguageModelV1

type NativeLLMCreator = <TOpts extends { baseURL: string; apiKey: string }>(opts: TOpts) => LanguageModelV1
